...
# =========================================================================
# ⚙️ Global Configuration (Scratch Version)
# =========================================================================
SKELETON_MODEL_PATH = "best_model_v3.pth"        # Skeleton model unchanged
RGB_MODEL_PATH = "best_model_rgb_scratch.pth"    # ⚠️ Load the newly trained scratch (non-pretrained) model
...
# =========================================================================
# 🛠️ General Data Processing Functions
# =========================================================================
def load_process_skeleton(npy_path):
    ...
    # Root normalization (hip center)
    root = (data[:, 23, :] + data[:, 24, :]) / 2
    data = data - root.reshape(frames, 1, 3)

    # Shoulder distance normalization
    ...
    # Padding or cropping to fixed length
    if data.shape[0] > 100:
...
class ValDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # Load video (avi or mp4 fallback)
...
def run_validation_and_search(skel_model, rgb_model):
    print(f"\n📊 [Scratch Comparison] Starting validation for the non-pretrained version...")
    ...
    with torch.no_grad():
        for i, (skel, rgb, lbl) in enumerate(val_loader):
            ...
            # Skeleton and RGB forward passes
    ...
    print("\n" + "="*60)
    print("🏆 [Scratch] Performance Comparison of Non-Pretrained RGB Stream")
    print("="*60)
    print(f"🦴 Skeleton Only: {skel_acc:.2f}% (Baseline)")
    print(f"🎨 RGB Scratch Only: {rgb_acc:.2f}% (Expected to be low)")
    print("-" * 60)
    print(f"🚀 Best Fusion Accuracy: {best_acc:.2f}%")
    print(f"⚖️ Optimal Weights: RGB={best_alpha:.2f}, Skeleton={1-best_alpha:.2f}")
    print("   (Expected: Fusion shifts heavily toward Skeleton stream)")
    print("="*60)
    ...
    print("✅ Saved confusion matrix: fusion_analysis_scratch.png")
...
if __name__ == "__main__":
    ...
    # Load Scratch RGB Model
    print(f"🧠 Loading Scratch RGB Model: {RGB_MODEL_PATH}")
    if not os.path.exists(RGB_MODEL_PATH):
        print("❌ Error: Scratch model not found, please run train_rgb_scratch.py first.")
        exit()
        
    rgb_model = models.r2plus1d_18(weights=None)  # ⚠️ Important: No pretrained weights
