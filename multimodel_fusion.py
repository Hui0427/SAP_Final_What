...
CSV_FILE = "annotations/train_set_labels.csv" # 必须存在，用于读取ID映射
...
def run_validation_and_search(skel_model, rgb_model):
    ...
    with torch.no_grad():
        for i, (skel, rgb, lbl) in enumerate(val_loader):
            ...
            all_skel_probs.append(s_out.cpu()); all_rgb_probs.append(r_out.cpu()); all_labels.append(lbl.cpu())
    ...
    print(f"\n🚀 最佳验证集结果: {best_acc:.2f}% (Alpha={best_alpha:.2f})")
    ...
    sns.heatmap(cm, annot=False, cmap='Blues', xticklabels=full_dataset.unique_labels, yticklabels=full_dataset.unique_labels)
    plt.xticks(rotation=90); plt.tight_layout(); plt.savefig('fusion_analysis_final.png')
...
def generate_submission(skel_model, rgb_model, best_alpha, int_to_label):
    print(f"\n📦 [Phase 2] 开始生成 3列格式 提交文件...")
    
    # 🌟 1. 构建 Label -> ClassID 的精准映射
    # 直接读取训练CSV，获取 [Name -> ID] 的关系
    train_df = pd.read_csv(CSV_FILE, header=None)
    # 假设CSV格式: Filename, LabelName, ClassID
    # 我们创建一个字典: {'Walking': 27, 'WalkingWithObject': 28, ...}
    label_to_id_map = dict(zip(train_df[1], train_df[2]))
    
    print(f"📋 已加载 {len(label_to_id_map)} 个类别的 ID 映射")
    
    # 2. 获取并排序测试文件
    test_files = sorted(glob.glob(os.path.join(TEST_VIDEO_DIR, "*.avi")) + glob.glob(os.path.join(TEST_VIDEO_DIR, "*.mp4")))
    print(f"📄 找到 {len(test_files)} 个测试视频")
    ...
    with torch.no_grad():
        for i, vid_path in enumerate(test_files):
            ...
            # 格式: Filename, LabelName, ClassID
            results.append([fid + ".avi", label_name, class_id])
            
            if (i+1)%50 == 0: print(f"  已处理 {i+1}/{len(test_files)}", end="\r")

    # 保存
    df = pd.DataFrame(results)
    # 老师的文件是3列，且没有header，我们保持一致
    df.to_csv(OUTPUT_FILE, index=False, header=False)
    
    print(f"\n\n🎉 提交文件已生成: {OUTPUT_FILE}")
    print("💡 格式示例: testvideo001.avi, Walking, 27")
